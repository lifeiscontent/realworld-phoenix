defmodule RealworldWeb.ArticleLiveTest do
  ...
  setup context do
    author = if context[:with_author] || context[:signed_in_as] == :author, do: user_fixture(), else: nil
    user = if context[:with_user] || context[:signed_in_as] == :user, do: user_fixture(), else: nil
    conn = case context[:signed_in_as] do
      :author -> log_in_user(context[:conn], author)
      :user -> log_in_user(context[:conn], user)
      _ -> context[:conn]
    end
    article = if context[:with_article], do: article_fixture(user: author), else: nil
